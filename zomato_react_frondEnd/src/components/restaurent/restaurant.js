...
function Restaurant() {
  ...
  let getRestaurantDetails = async () => {
    let url = "http://localhost:5003/api/restaurantlistbyid/" + id;
  ...
  let getMenuItems = async () => {
    let url = `http://localhost:5003/api/menuitembyid/${id}`;
    let { data } = await axios.get(url);
    console.log(data);

    if (data.status === true) {
      setMenuList([...data.menuItems]);
    } else {
      setMenuList([]);
    }
    setTotalPrice(0)
  ...
  let makePayment = async () => {
    ...
    var options = {
      ...
      image:"https://www.freelogovectors.net/wp-content/uploads/2016/12/zomato-logo-785x785.png",
